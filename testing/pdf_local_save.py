...
def initialize_session_state():
    ...
    session_vars = {
        ...
        "processed_pdfs": [],
        "quality_level": 3,
        "conversion_folders": [],  # Keep track of conversion folders
        "current_conversion": None  # Track current conversion session
    }
...
def get_unique_folder_name():
    ...
    return f"conversion_{timestamp}"

def save_images_locally(processed_images, pdf_name):
    """Save processed images to local directory"""
    # Create a unique folder for this conversion
    folder_name = get_unique_folder_name()
    conversion_dir = OUTPUT_DIR / folder_name
    ...
    for img_dict in processed_images:
        ...
        img_path = conversion_dir / img_dict['name']
        
        # Save the image
        img_dict['data'].seek(0)
        with open(img_path, 'wb') as f:
            f.write(img_dict['data'].getvalue())
        
        saved_paths.append(img_path)
    
    # Update session state with new conversion folder
    if 'conversion_folders' not in st.session_state:
        st.session_state.conversion_folders = []
    st.session_state.conversion_folders.append(conversion_dir)
    st.session_state.current_conversion = conversion_dir
    
    return conversion_dir, saved_paths
...
def convert_pdf_to_images(pdf_file, quality_level):
    ...
    try:
        ...
        # Save images locally and get the save location
        if processed_images:
            save_dir, saved_paths = save_images_locally(processed_images, pdf_file.name)
            st.sidebar.success(f"Images saved to: {save_dir}")
            
            # Add download buttons for each image
            with st.sidebar.expander("Download Converted Images"):
...
def main():
    ...
    Each conversion creates a unique timestamped folder.
    """)
